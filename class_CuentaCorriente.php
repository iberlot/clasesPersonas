...
class CuentaCorriente {
    ...
    function ctacteReal() {
        ...
        if ($vencimiento_matricula) {
            ...
            foreach ($vencimiento_matricula as $key => $value) {
                ...
                $IMPORTE_MATRICULA[$i] = number_format((float) $matriculas_array[$i - 1], 2, ',', '');
        ...
        if ($id_aranceles) {
            ...
            foreach ($vencimiento_aranceles as $key => $value) {
                ...
                if(isset( $aranceles_array[$cont- 1])){
                    $importeatancel[$cont] = number_format((float) $aranceles_array[$cont- 1], 2, ',', '');
                }else{
                     $importeatancel[$cont] = number_format(0, 2, ',', '');
                }
                ...
                if (($importeatancel[$cont] > 0) and ( $importeatancel[$cont] < 999999)){
                    ...
                    $this->setArancelTotal($importeatancel[$cont] .'----'.$recargo.'---'.$recargo_arancel_ftermino);
    ...
    function setCta_Matricula($Cta_Matrï¿½cula) {
        $this->Cta_Matriccula[] = $Cta_Matrï¿½cula;
    }
