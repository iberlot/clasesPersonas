...
class CajaTesoreria
{
	...
	function __construct($db, $idsesion = null)
	{
		...
		if ($idsesion != null)
		{
			...
			$query = "select * from SESIONCAJA JOIN " . " SESIONDETALLE ON SESIONCAJA.IDSESION = SESIONDETALLE.IDSESION" . " WHERE SESIONCAJA.IDSESION = :idsesion ";
	...
	function iniciarSesionCaja($datos, $fecha)
	{
		...
		$query = "INSERT INTO SESIONCAJA" . "(IDSESION,IDPERSON,IDSESIONUSUARIO , FECHAAPERTURA,NROCAJA,TURNO,FONDOAPERTURA ,NOMBREUSUARIO ,IDSUCURSAL )VALUES(" . "tesoreria.sesioncaja_seq.nextval , :idperson , :idsesionusuario ,TO_DATE('$fecha', 'dd/mm/yy'), :nrocaja , :turno," . ":fondoapertura , :nombreusuario ,:idsucursal)";
	...
	function getBalanceCaja()
	{
		...
		$query = "select ESTADO , IMPORTE , RECARGO , FTERMINO FROM movimientos " . "WHERE ESTADO IN (2,3,4)
